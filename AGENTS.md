...
- Quality gates pass before any commit or push: `CI=1 bun run verify` green. The git pre-commit hook in `.githooks/pre-commit` runs that command and blocks the commit on failure; after clone run `bun run hooks:install` once so `core.hooksPath` points at `.githooks`. Do not bypass with `--no-verify`. CI runs the same verify (includes build) plus the browser and native harness steps. The test workflow triggers on `pull_request` (not `pull_request_target`) so checkout exercises the PR head. Verify's wave-1 typecheck (`lib/tsconfig.typecheck.json`) covers library entry points and all `src/**/*.spec.ts(x)` files; wave 3 runs `typecheck:harness` for the browser and native workspaces against built package exports.
- Lint and format with Biome (`bun run lint` / `bun run format`); Prettier is not used. Framework lint domains are off at the root and enabled per path in `biome.json` (react, reactNative, solid, vue, test, playwright) because react and solid conflict.
- Unexpected `console.warn` / `console.error` fail the unit suite via `lib/src/__mocks__/console-leak.ts` unless the message is an allowlisted DEV sanitizer warning. Tests that assert on warnings keep their spies.
...
- Aesthetics and UX: the library is headless. The `site/` workspace is a demo; changes there are judged by rendering and viewing them, never asserted in a unit test. Do not change site visuals (shader, palette, layout, motion) without explicit consent, and do not bury a look change under an unrelated PR.
...
- `bun run verify` (`scripts/verify`) is the shared local and CI quality gate. Wave 0 runs Biome with `--write --unsafe` then a post-autofix diff audit (CI fails if Biome rewrote the tree). Wave 1a fans out in parallel: knip, typecheck (lib only), validate-i18n, `timeout 5 bun test --coverage` (80% funcs and lines floor on All files), and redos-smoke. Wave 1b runs metrics-gate for all seven targets serially (3-run median) so CPU contention does not inflate the median. Its ceiling is deliberately loose, sized to catch an extreme regression on hardware slower than the machine that recorded the baseline; read small movements from `bun metrics` on a stable machine instead. Wave 2 builds. Wave 3 fans out: typecheck:harness (browser/native against lib/dist exports), emit-selfcheck (dev and production), prod-strip-check, hermetic-dist, llms-snippets. Browser and native harness smoke tests stay outside verify as separate CI steps. Pre-commit runs the same gate with `CI=1` via `.githooks/pre-commit` after `bun run hooks:install`.
...
- The `browser/` workspace is the live DOM harness for the React, Solid, and Vue builds (production dist via the package exports map). `bun dev:browser` builds lib and serves Vite on 4174; `bun test:browser` builds lib plus the harness bundle and runs Playwright against both Vite preview (4173, production) and Vite dev (4174, development) so DEV-mode warnings are visible; `bun test:browser:built` skips both builds. Both harness workspaces are pruned from `bun test` discovery by `bunfig.toml` `pathIgnorePatterns`.
- `fixtures/cases.ts` is the single home of the shared harness corpus: one entry per markdown feature and issue regression, each with renderer-agnostic DOM expectations. Optional `options` (for example `optimizeForStreaming`) and `skip: ['browser'|'native']` gate cases that are entry-point or platform specific. The browser page mounts every non-skipped case per renderer and the Playwright suite asserts the expectations; the native workspace consumes the same list for its showcase and jest smoke coverage. Expectations assert tags, counts, ids, hrefs, and exact text, never classes, inline styles, or whitespace, and every image source is a data URI so the harness stays hermetic.
- The `native/` workspace is the React Native harness: an Expo app consuming lib source through Metro `watchFolders` (edits hot-reload), plus a `jest-expo` suite validating the built package. `bun dev:native` starts Metro; `bun test:native` builds lib and runs jest; `bun test:native:built` skips the lib build. The Fast Refresh loop needs Watchman installed (`brew install watchman`, then restart Metro) and `CI=1` left unset; cold reload is `xcrun simctl terminate <udid> host.exp.Exponent` then `simctl openurl <udid> exp://127.0.0.1:8081` (foregrounding alone serves the stale bundle).
...
- `lib/README.md` is the single source for library docs. Any edit propagates in the same change to every `lib/src/i18n/{lang}/README.md`: copy it verbatim into `en/README.md` as an exact mirror, and translate the changed sections into `hi` and `zh` per their `.cursor/rules/i18n-{lang}.mdc`. `validate-i18n` checks UI strings, required files, equal README fence counts, TOC entry count parity with `lib/README.md`, TOC anchors resolving to headings or HTML ids in the same file, and exact TOC anchor match for `en/`. Keep code identifiers, API names, and anchors as each file already does.
