...
- A new inline scanner gets a case in `scripts/scaling-smoke.ts`, not only a payload in `scripts/redos-smoke.ts`: a quadratic passes an absolute budget whenever the size someone picked is still cheap, which is how the footnote scanner shipped quadratic.
