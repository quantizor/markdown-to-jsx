...
- Adjacent capture groups with overlapping char classes (e.g., `[a-zA-Z0-9]*` then `[^>]*`) cause backtracking when the delimiter (`>`) is absent; replace with charCode scans that parse tag name then attributes in a single forward pass.
- Regexes applied to unbounded library input (e.g., entire document via `.match()`) are inherently risky; prefer charCode scans that walk the string once with no backtracking.
