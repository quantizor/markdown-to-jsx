...
Support every option that makes sense for the output format. If an option is not supported, it should be marked as optional+never in the exposed option type for that integration and be noted in the documentation. Add the target to `scripts/metrics.ts` and `scripts/profile.ts`.
