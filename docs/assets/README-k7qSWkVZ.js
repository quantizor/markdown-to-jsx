...
- [用法 Usage](#%E7%94%A8%E6%B3%95-usage)
  ...
  - [库选项 Library Options](#%E5%BA%93%E9%80%89%E9%A1%B9-library-options)
    ...
    - [原始 HTML 清理 Raw HTML sanitization](#%E5%8E%9F%E5%A7%8B-html-%E6%B8%85%E7%90%86-raw-html-sanitization)
...
React Native 的输出自带一份简洁、精简的基础样式表，使 Markdown 开箱即用地呈现出清晰的层次结构：标题字号级联、等宽字体代码、块级间距、引用竖线，以及带表头和对齐列的表格。所有内容都可自定义。\`styles\` 中的每个键都会与对应元素的默认样式逐属性合并，因此只设置一个属性（例如 \`heading1.color\`）时，其余默认值会保留。\`styles.text\` 是应用于所有渲染文本之下的基础样式，因此您可以一次性为整个文档设置字体、颜色和大小：

\`\`\`tsx
<Markdown options={{ styles: { text: { color: '#333', fontSize: 17 } } }}>{content}</Markdown>
\`\`\`

通过键覆盖任意元素。每个键都是可选的，且会与默认值合并，因此您只需修改想要更改的属性：

\`\`\`tsx
<Markdown
  options={{
    styles: {
      heading1: { color: '#b91c1c' }, // 只修改一个属性，其余保留默认值
...
可用的键如下，每个键都对应其目标组件所接受的样式类型（\`TextStyle\`、\`ViewStyle\` 或 \`ImageStyle\`）：

- 文本：\`text\`（应用于所有渲染文本的基础样式）、\`paragraph\`、\`heading1\` 到 \`heading6\`、\`link\`、\`footnote\`、\`codeInline\`、\`strong\`、\`em\`、\`del\`、\`mark\`、\`listItemBullet\`、\`listItemNumber\`
- 块级元素：\`blockquote\`、\`codeBlock\`、\`thematicBreak\`、\`image\`
- 列表：\`listOrdered\`、\`listUnordered\`、\`listItem\`
- 表格：\`table\`、\`tableHeader\`、\`tableHeaderCell\`、\`tableHeaderText\`（加粗的表头文本）、\`tableRow\`、\`tableCell\`、\`tableCellDivider\` 和 \`tableRowDivider\`（网格线）
- GFM 任务：\`gfmTask\`（绘制的复选框）、\`gfmTaskChecked\`（选中状态的强调填充）、\`checkmark\`（对勾字形）

原始 HTML 容器标签（\`div\`、\`section\`、\`article\`、\`ul\`、\`ol\`、\`li\`、\`th\`、\`td\` 等）也可以用自己的标签名设置样式。

要完全替换某个元素的渲染方式而非仅重新设置样式，请使用 \`overrides\`。下面是一个结合两者的完整主题配置示例，并附带一个将围栏代码替换为语法高亮器的 \`renderRule\`：
...
围栏代码块渲染为 \`<pre><code>\`，且这两个元素都不会接收到围栏的语言信息，因此需要感知语言的代码渲染（语法高亮、为 \`\`\`\` \`\`\`latex \`\`\`\` 渲染 KaTeX 代码块）应放在 \`renderRule\` 中处理，此处可以获取 \`node.lang\` 和 \`node.text\`。
...
任务复选框（\`- [x]\`、\`- [ ]\`）通过一个默认映射为 \`View\` 的 \`<input type="checkbox">\` 标签渲染，呈现为一个绘制的复选框：一个带描边的方框，任务完成时会填充强调色并显示对勾。该复选框取代了列表项符号的位置，任务项的列表符号会被隐藏。使用 \`styles.gfmTask\` 重新设置方框的样式，或覆盖 \`input\` 完全替换复选框的视觉表现（例如使用真实的 \`<Image>\` 复选框、添加动画状态等）。你的覆盖组件会接收 \`checked\`、\`type: 'checkbox'\`、\`readOnly\`、合并后的 \`style\`，以及一个渲染 \`[x]\` 或 \`[ ]\` 作为后备标记的 \`<Text>\` 子元素；打算完全自定义视觉表现的使用者应忽略该子元素，转而基于 \`props.checked\` 自行渲染指示器。

任务项外层的列表项包装器默认会应用 \`flexDirection: 'row'\` 和 \`alignItems: 'flex-start'\`，让复选框与标签的第一行对齐；复选框自身的顶部外边距会将其在该行内居中，因此即便标签换行为多行也能保持位置正确。传入自定义的 \`styles.listItem\` 即可覆盖这些值——\`mergeStyle\` 会保留行布局默认值在底层，因此你设置的任何属性在冲突时都会胜出（例如传入 \`alignItems: 'center'\`，即可让复选框相对于整个标签垂直居中）。
...
**混合文本与块级内容：** React Native 无法在文本中嵌套图像或视图，因此当内联容器（段落、标题、强调或链接）包含图像或块级内容时，它会渲染为 \`View\` 而非 \`Text\`。其中的文本会归入一个 \`Text\`，以便仍在同一行中排布；图像作为独立元素渲染；链接内的图像则通过 \`Pressable\` 保持可点按。纯文本内容不受影响，渲染方式与之前一致。

**注意：** 为了更好的可访问性和可发现性，链接默认带有下划线。您可以通过 \`styles.link\` 选项覆盖此行为。

**注意：** 脚注引用标记（\`[^1]\`）渲染为上标，与 Web 渲染器输出的 \`<sup>\` 一致。React Native 无法通过样式让内联文本升高或缩小（会忽略内联文本上的 \`verticalAlign\` 和变换），因此数字标记使用 Unicode 上标字形（\`¹²³\`），字体会自行以相对周围文本升高、缩小的方式绘制这些字形。非数字标识符（\`[^note]\`）则渲染为纯文本。可通过 \`styles.footnote\` 选项设置该标记的样式。
...
#### 原始 HTML 清理 (Raw HTML sanitization)

您的 markdown 中的原始 HTML 会在每个渲染器 (React、React Native、HTML、Markdown、Solid 和 Vue) 中自动清理，因此不受信任的输入无法将脚本潜入您的输出。此功能始终启用，且独立于 \`options.sanitizer\`，后者仅控制 URL scheme。

渲染前移除：

- 内联事件处理程序：\`onclick\`、\`onerror\`、\`onload\` 以及任何其他 \`on*\` 属性。
- 在 \`href\`、\`src\`、\`action\`、\`formaction\`、\`poster\`、\`cite\`、\`background\`、\`data\`、\`longdesc\` 和 \`xlink:href\` 中携带 \`javascript:\`、\`vbscript:\` 或非图像 \`data:\` scheme 的 URL 属性。隐藏在 HTML 实体后面的 scheme，例如 \`java&#9;script:\` 或无分号的 \`&#106avascript:\`，也会被解码并捕获。
- \`iframe\` 的 \`srcdoc\` 属性。

保留：保持原始格式的安全属性、\`data:image\` URL，以及您作为表达式传递给自己组件的事件处理程序 (\`<MyButton onClick={fn} />\`)，以及它们上面的裸布尔属性 (\`<MyButton onClick />\`)。一个注意事项：\`data:image/svg+xml\` 是允许的，但当作为顶级导航打开时可能会执行脚本，因此请谨慎处理来自不受信任来源的 SVG data URL。

危险的标签名称 (\`script\`、\`iframe\`、\`style\` 等) 由 [\`tagfilter\`](#all-options) 选项单独转义，该选项默认启用。
...
- 未闭合的删除线（\`~~文本\` 没有闭合 \`~~\`）
- 未闭合的链接（\`[文本](url\` 没有闭合 \`)\`）
- 不完整的表格（在第一行数据到达前只有表头行或分隔行）；表格一旦渲染就会保持不变，即便后续行继续流入也是如此，因此不会闪现原始的竖线字符，也不会在行与行之间闪烁
